/**
 * Edit Program Dialog
 * For Super Admins to edit existing educational programs
 */
...
export default function EditProgramDialog({ open, onOpenChange, program }: EditProgramDialogProps) {
  ...
  const updateProgram = useMutation(api.program.updateProgram);
  ...
  useEffect(() => {
    if (program) {
      setFormData({
        ...
        timetable: program.timetable as Record<string, TimetableSlot[]>,
      });
    }
  }, [program]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit the details of the educational program.
          </DialogDescription>
        </DialogHeader>
